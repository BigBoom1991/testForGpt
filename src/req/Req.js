...
const options = {
    ...
    chat: { value: 'chat', label: '对话', disabled: true },
    images: { value: 'images', label: '创建图像' },
    editImages: { value: 'editImages', label: '修改图像', disabled: true },
}
const sizeOptions = [{lable:"256x256",value:"256x256"},{lable:"512x512",value:"512x512"},{lable:"1024x1024",value:"1024x1024"}]
...
class Req extends React.Component {
    ...
    req = async () => {
        ...
        switch (this.state.type) {
            ...
            case options.images.value: {
                var completion = await openai.createImage({
                    prompt: this.state.prompt,
    ...
    render() {
        let { token, param, prompt, maxToken, output, resToken, reqToken,user,pictureNum } = this.state;
        return (
